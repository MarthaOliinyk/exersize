...
class SubscriptionType(db.Model):
    ...
    def get_filtered(cls, data):
        filtered = db.session.query(SubscriptionType).join(Course).filter(Course.tag == data['tag']) \
        ...
        return {'filtered': [to_json(s_type) for s_type in filtered]}
